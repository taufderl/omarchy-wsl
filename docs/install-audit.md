...
| `omarchy-provision-owner.service` | Adapted (`wsl/omarchy-wsl-provision-owner.service`) | Vendored upstream copy kept at `patches/omarchy-provision-owner.service.upstream` for reference/diffing. The real unit is gated on `ConditionPathExists=/var/lib/omarchy/provisioning/pending` and **is not enabled by default even on bare metal** (upstream's own comment: *"Not shipped enabled — a normal install never runs it"* — it's the deferred-provisioning/factory-reset path). We reuse that same gating pattern, arm it at image-build time (see `wsl/arm-first-boot.sh`), and drop the `plymouth quit` `ExecStartPre` and the `Before=display-manager.service`/`Conflicts=getty@tty1.service` ordering against units (`sddm`, `plymouth`) that aren't installed in this image. |
